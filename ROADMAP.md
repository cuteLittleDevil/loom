...
协程池已按 `docs/design/pool.md` 实现。调度没有互斥锁，也没有 `Close`：协调者和巡检随进程存在。对外入口在 `pool.go`，协调者、执行者和告警分别在 `coordinator.go`、`executor.go`、`alert.go`。`go.mod` 要求 Go 1.27，toolchain 为 go1.27.1。设计文档的 Status 仍是 Draft。
