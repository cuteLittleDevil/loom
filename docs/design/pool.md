Title: 固定 worker 执行泛型任务，池满按优先级等待，占用超时只告警
Author(s): cd
...
`loom` 提供一个只依赖标准库的固定容量协程池。`Submit` 是 `Pool` 上的泛型方法，立刻返回 `<-chan Result[R]`，不阻塞到任务结束。用户函数的形状是 `func() (R, error)`，只在 worker 上执行。channel 容量为 1，恰好送出一次 `Result` 然后关闭；没人接收也不会堵住 worker。`Result` 里的快照是任务终态并完成分派的那一刻，不是调用方后来执行 `<-ch` 的那一刻。池满时任务进入优先级队列；调度在同一把池锁里完成，数值更大的优先级先执行，同优先级按任务 ID。正在执行的任务不会被抢占。worker 连续执行超过阈值时，池子在任务仍被巡检看到的期间发出告警。最重要的承诺是：并发上限等于配置的 worker 数，告警不改变任务的控制流。
...
### 池子在创建时启动固定数量的 worker，并发上限就是这个数量
...
`Size` 是 worker 数。`New` 成功时立刻启动这 `Size` 个 goroutine，不再为任务或结果投递额外创建 goroutine。告警开启时再启动一个巡检 goroutine。`Size <= 0` 时 `New` 返回 `nil, ErrInvalidSize`，不启动任何 goroutine。
...
池内调度状态用一把互斥锁保护。用户函数和 `OnAlert` 都在这把锁之外执行。`Submit`、`Close`、巡检和回调可以并发。

`Submit` 本身立刻返回，在回调或用户函数里调用它不会占住 worker。下面两种同步等待会死锁，池子不为此加 worker，也不代为取消：

- 用户函数里接收同一个池的 `Submit` channel。外层任务占着 worker，满员时内层任务排不上。
- 回调或用户函数里调用 `Close`。`Close` 在等待时不持有池锁，但会等到 worker 和巡检退出，于是和这次等待互等。

### Submit 立刻返回 channel，worker 结束后送出一次 Result
...
channel 的容量是 1。进入终态并复制快照之后，worker 解锁，发送这一次 `Result`，然后 `close`。发送发生在执行下一个用户函数之前，所以调用方不会被下一个任务的耗时拖住。容量为 1，没人接收时发送也不阻塞，worker 不会因此少一个。第二次接收得到零值 `Result` 和 `ok == false`，那不是一次任务结果。

调用方的 goroutine 不被 `Submit` 占住。调度只有一条路：在池锁里接受任务，由 worker goroutine 执行。不存在「调用方看到空闲 worker 就自己跑函数」或「新任务抢走已经排着队的位置」的快路径。

接受发生在池锁里，并且和关闭检查是同一个临界区：

1. 已 `Close` 则拒绝，任务不存在。
2. 否则分配任务 ID，`Submitted` 加 1。有空闲 worker 时，这个任务直接标成 `Running` 并交给它；没有时，任务进入优先级队列。有空闲 worker 时队列一定是空的，所以直接标成 `Running` 不会插队。

任务 ID 是从 1 起、每次加 1 的 `uint64`，不复用。被拒绝的提交不占 ID。同优先级按 ID 从小到大执行，不使用时间戳。ID 就是提交顺序。

函数 panic 时，worker 恢复现场并继续取任务。`Result.Value` 是零值，`Result.Err` 是 `*PanicError`。`errors.Is(err, ErrPanic)` 为真，`errors.As` 可取出 panic 的原始值和栈。栈是 `recover` 处 `debug.Stack` 的结果，属于 worker goroutine，不是接收 channel 的那条 goroutine。错误文本不构成兼容承诺。
...
`RunningTasks` 只包含已标成 `Running`、尚未终态的任务。等待队列可能很长，送出结果时不复制整份队列。等待侧只有 `Waiting` 和 `WaitingBy`。`WaitingBy` 和 `RunningTasks` 在锁内新分配，调用方可以一直持有。时长用 `time.Since` 一类单调钟计算，不用墙钟相减。

快照和 `Alert` 都在不变量已经成立的临界区里复制，然后才解锁。快照描述的是复制那一刻。调用方可以很晚才接收，期间调度还会变，这份快照不更新。它也不承诺之后还有空位。

锁释放时下列式子成立：

- `Waiting > 0` 时 `Idle == 0`。有人排队就没有空闲 worker。
- 未 `Close` 时 `Idle + Running == Size`。
- 已 `Close` 时 `Idle + Running <= Size`，差值是已经退出的 worker。已退出的不算空闲。
- `Running + Waiting == Submitted - Completed`。

因此，一个任务结束时，同一个临界区里先把它计入终态，再按优先级把下一个排队任务标成 `Running`，然后才复制快照。队列非空时，这份快照不会把这个 worker 算成空闲；刚结束的任务自己不在 `RunningTasks` 中，下一个任务已经在里面。队列为空时，这个 worker 还没退出，快照里算作 `Idle`，和关闭标志无关。快照复制之后，未关闭就回去等任务，已关闭就退出。所以排空中最后一条任务的 `Result` 仍可能 `Idle >= 1`；`Close` 返回之后再 `Submit`，收到的快照才会是 `Idle == 0`。

`Idle > 0` 只说明采集时没有排队任务。未关闭时，随后的 `Submit` 不需要等别的任务结束，但仍然由 worker 执行。已关闭时，`Idle` 不表示还能提交。

```go
func (p *Pool) worker() {
    for {
        task := p.wait() // 没有已分派任务时阻塞；已 Close 且无任务则返回 nil
        if task == nil {
            return
        }
        p.execute(task)          // 用户函数在锁外执行；panic 恢复成 *PanicError
        snap := p.finish(task)   // 锁内：终态、分派下一个、复制快照
        task.deliver(snap)       // 解锁后发送一次并 close；容量 1，不阻塞
    }
}
```
...
`p == nil`、`fn == nil` 在加锁之前拒绝。`p == nil` 优先于 `fn == nil`。两者都返回已经放好一次 `Result` 的 channel：`Err` 是对应哨兵，`Value` 和 `Snapshot` 都是零值，不读取池子，计数器不变。

池已 `Close` 的拒绝在池锁里进行。channel 同样已经放好一次 `Result`：`Err` 是 `ErrClosed`，`Snapshot` 是锁内实况，计数器不变，任务不接受。`Submit` 调用本身仍然立刻返回。
...
worker 都在执行时，新任务进入优先级队列。`Submit` 仍然立刻返回。调用方要等结果，就在自己的 goroutine 上接收 channel。队列没有容量上限，也没有取消入口。不为排队任务再开 goroutine。
...
每个 `Running` 任务记录标成 `Running` 的时刻。巡检 goroutine 只在告警开启时存在。它在池锁里查看这些时刻，复制告警内容并更新计数，然后解锁，再串行调用 `OnAlert`。
...
巡检间隔是 `min(T, time.Second)`。这只约束巡检自己醒来的间隔：`T >= 1s` 时，决策时刻最多比到期点晚大约 1s；`T` 更短时，最多晚大约一个 `T`。`OnAlert` 在锁外按任务 ID 从小到大串行调用，回调花的时间会继续推迟后面的回调和下一次巡检。这个推迟不计入上面的间隔上界。

`At` 和 `RunningFor` 取决策临界区里的值，不包含排在前面的回调耗时。`Running` 包含正在告警的这条任务。`Idle`、`Running`、`Waiting` 满足前面的不变量。回调真正执行时，任务可能已经结束；`Alert` 描述的是决策时刻。
...
同一轮里先在锁内为所有到期任务更新 `k`、池的 `Alerted` 计数和 `TaskInfo.Alerted`，再解锁回调。因此回调执行期间，别的 `Result` 已经能看到这次告警。`OnAlert` 为 nil 时同样更新，只是跳过调用。回调 panic 由巡检恢复，巡检继续处理本轮剩余任务和后续巡检；这次告警已经计过数，不撤回。

告警和调度是两条路。`OnAlert` 不取消任务，不调整优先级，不把 worker 从任务里拽出来。调用方若只看自己的 `Result`，可以读 `Snapshot.RunningTasks[].Alerted` 和 `Snapshot.Alerted`。那是终态那一刻的记录；任务还在跑的时候，通知靠 `OnAlert`。

池未满时同样巡检。一个 worker 卡住时，即使还没有人在排队，占用事实已经成立。

### Close 拒绝新任务，并等待已经接受的任务结束

`Close` 在池锁里置上关闭标志，唤醒阻塞中的 worker 和巡检，然后释放锁，再等待。等待的范围是：置位之前已经接受的任务全部进入终态，对应的 `Result` 已经发送，worker 退出；若巡检存在，则它也退出。不调用 `Close` 时，这些 goroutine 不退出。巡检把退出检查放在回调之外：已经 `Close` 且 `Running == 0`、`Waiting == 0` 就退出。正在执行的回调不打断，返回后再检查。唤醒是为了让排空不必睡满当前巡检间隔。因此在调用方不从回调或用户函数里同步等待 `Close` 的前提下，`Close` 返回时，排空期间决定发出的 `OnAlert` 已经返回。
...
关闭完成之后再 `Submit`，收到的快照里 `Idle == 0`、`Running == 0`、`Waiting == 0`，`Size` 仍是配置值。排空过程中，已经退出的 worker 不算 `Idle`。

## Rationale / 理由与取舍

### 并发上限由池子拥有的 worker 实现，调用方自己决定要不要等
...
固定 worker 把「谁在执行」收成最多 `Size` 条记录。占用告警扫的就是这张表。

`Submit` 若阻塞到终态，每个排队调用都占住一条调用方 goroutine。返回容量为 1 的 channel 之后，等待变成调用方的选择：不接收就不占 goroutine，接收才等。worker 的发送不能再阻塞，否则不接收的调用方会偷走一个并发名额。

用户函数里同步接收同一个池的 channel 时，当前 worker 被外层任务占着；没有空闲 worker 时内层任务排不上，形成永久等待。这和信号量嵌套占用是同一类用法错误。池子不偷偷扩容，因为扩容会直接破坏并发上限。只调用 `Submit`、不接收，不会形成这个等待。
...
我们放弃「有空闲 worker 时调用方直接执行」以及「结束任务后先把 worker 算成空闲、解锁、再去领下一个」。那个窗口里队列可以非空而 `Idle > 0`，新的 `Submit` 会抢走本该给排队任务的 worker。所以用户函数只跑在 worker 上，分派和快照复制放在同一个临界区，锁释放时 `Waiting > 0` 必然 `Idle == 0`。代价是调用方看不到「刚空出来、但马上又被队列占上」的那个瞬态。
...
快照钉在终态临界区，而不是钉在 `<-ch`。接收可以发生在很多个任务之后，若那时再拍一张，调用方就分不清自己这次任务结束时池子是什么样。
...
现成池子的提交口通常是 `func()`，返回值要调用方自己塞 channel，优先级和占用告警也要在外面再包一层。那一层包完，和自己写一个小池子的代码量接近，还多一个核心依赖。这个池子的调度、告警和快照都在标准库能表达的范围内：`container/heap`、`sync`、`time`、`runtime/debug`。方法上的类型参数使用 Go 1.27 的语言能力，不是第三方包。
...
- `Submit` 不占调用方 goroutine。1000 个排队任务是 1000 个闭包和 1000 条容量为 1 的 channel，外加最多 `Size` 条 worker；告警开启时再加 1 条巡检。接收 channel 的 goroutine 由调用方自己决定。
...
1. `New`、固定 worker、泛型方法 `Submit`、容量为 1 的一次发送、`PanicError`、计数器，以及「终态后在同一把锁里分派下一个，再复制快照，解锁后发送」。
2. 优先级堆。只从队首取，不按任务删除。
3. 巡检 goroutine、按周期合并的 `OnAlert`、回调 panic 恢复、`Alerted` 计数。
4. `Close` 拒绝新任务，等待已接受任务终态并且 `Result` 已发送，再等待 worker 和巡检退出。
...
- 调用方不接收时，worker 仍能执行下一个任务。
- `Size` 为 1 时，先占住唯一 worker，再按优先级 1、10、5 提交三个任务。占住的任务放开之后，三个任务的开始顺序是 10、5、1。占住的任务的 `Result` 里，优先级 10 已经在 `RunningTasks` 中，`Idle == 0`，`Waiting == 2`。
...
- 函数 panic 后，`errors.Is` 识别 `ErrPanic`，`errors.As` 得到原来的值和栈，同一个 worker 还能执行下一个任务。
- `p == nil`、`fn == nil` 时，channel 在 `Submit` 返回前就已经放好对应哨兵和零值快照，不改变已有池的计数器。两者都为 nil 时是 `ErrNilPool`。
- `Close` 之后的 `Submit` 得到 `ErrClosed` 和实况快照。`Close` 之前已接受的任务仍会执行并送出 `Result`。并发的再次 `Close` 等到同一次排空结束后返回 nil。排空结束时 worker 和巡检都已退出。
...
| `p == nil` | 已放好的 channel，`ErrNilPool`，零值快照，不加锁 |
| `fn == nil` 且池非 nil | 已放好的 channel，`ErrNilFunc`，零值快照，不加锁 |
...
`RunningFor` 在采集快照或决定告警时用 `time.Since` 计算，不是任务结束后的账单，也不是 `<-ch` 那一刻的时长。同一个任务在告警和在稍后的 `Result` 里，时长不同。`WaitingFor` 在标成 `Running` 时冻结，是从接受到开始执行的排队时间；之后不再增加。没进过队列、直接标成 `Running` 的任务，这个值只覆盖持锁分派本身的时间，不会被另记成 0。
