...
任务 ID 是从 1 起、每次加 1 的 `uint64`，不复用。被拒绝的提交不占 ID。同优先级按 ID 从小到大执行，不使用时间戳。ID 就是提交顺序。降级执行的任务会占掉一个 ID，避免稍后入队的任务插到更早的提交前面，但这个 ID 不会出现在运行表里。
...
| `Degrade` 返回 `true`，在池外执行 | 不变 | 不变 | 不变 |
...
`Config.Degrade` 可以改掉这一步。它在没有空闲槽位、任务即将排队时调用，参数是当时已标成 `Running` 的任务，用来看它们是否一直占着槽位。返回 `true` 时，这次任务不进池，池子另开协程直接执行用户函数。这次执行不占用 `Size`，不出现在运行表里，不计入 `Submitted`、`Completed`、`Failed`。`Result` 仍从原来的 channel 送出，快照是执行结束时池子的实况。返回 `false` 或 `Degrade` 为 nil 时，任务进入优先级队列。回调在协调者之外执行。不要在里面同步接收本池的 channel。回调 panic 时，这次任务改为入队。
